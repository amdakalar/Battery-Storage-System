...
import { XMarkIcon, PlusIcon, CalendarIcon, Square3Stack3DIcon } from '@heroicons/react/24/outline';
import { getTodayISODate } from '../utils/dateUtils';
import { DroneCategoryInfo, loadCategories, DEFAULT_CATEGORY } from '../constants/categories';
...
export const AddBatteryModal: React.FC<AddBatteryModalProps> = ({
  ...
  const [category, setCategory] = useState(defaultCategory || activeCategories[0]?.id || DEFAULT_CATEGORY);
  ...
  React.useEffect(() => {
    if (isOpen) {
      setName('');
      setCategory(defaultCategory || activeCategories[0]?.id || DEFAULT_CATEGORY);
      setLastChargeDate(getTodayISODate());
      setReminderIntervalDays(40);
      setNotes('');
      setVoltage('');
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[14px] font-bold text-slate-800 leading-tight">زیادکردنی باتری نوێ</h3>
              <p className="text-[11px] text-slate-400 font-medium mt-0.5">داخڵکردنی زانیارییەکان لە بەشی هەڵبژێردراو</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 min-h-0">
          <form id="add-battery-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>ماوەی ئاگادارکردنەوە (ڕۆژ)</label>
                <input
                  type="number"
                  min="1"
                  max="365"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>کۆی ڤۆڵتی باتری (V)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  ...
                  placeholder="نموونە: 22.8"
                  className={inputCls}
                />
                <p className="text-[10px] text-slate-400 mt-1.5">خۆکار لەگەڵ سێڵەکان دادەنرێت</p>
              </div>
            </div>

            {/* Notes */}
            <div>
              <label className={labelCls}>تێبینی (ئارەزوومەندانە)</label>
              <input
                type="text"
                value={notes}
