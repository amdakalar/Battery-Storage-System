  ...
  const handleExportExcel = () => {
    ...
    const rows = batteries.map((b, idx) => {
      ...
      return [
        ...
        `"${formatCellVoltagesText(b)}"`,
        `"${formatGregorianKurdish(stats.nextDueDate)}"`,
        `"${stats.statusText}"`,
      ];
    });
...
<head>
  <meta charset="UTF-8"/>
  <title>ڕاپۆرتی ستۆرجی باترییەکان — ${formattedToday}</title>
...
<body>
  ...
  <div class="page-header">
    ...
    <div class="meta-block">
      <div>بەرواری چاپ: <strong>${formattedToday}</strong></div>
  ...
  <div class="page-footer">
    <span>سیستەمی بەڕێوەبردنی ستۆرجی باترییەکان — ڕاپۆرتی فەرمی</span>
    <span style="font-family:monospace;">${today} — ${nowTime}</span>
  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200 dir-rtl">
      <div className="bg-white rounded-2xl max-w-5xl w-full shadow-2xl border border-slate-100 relative max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {/* Footer note */}
          <div className="flex items-center justify-between pt-2 border-t border-slate-200 text-[9.5px] font-bold text-slate-900 mt-1">
            <span>سیستەمی بەڕێوەبردنی ستۆرجی باترییەکان — ڕاپۆرتی فەرمی</span>
            <span className="font-mono">{today} — {nowTime}</span>
          </div>
