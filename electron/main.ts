 ...
 * - Dev:        ../public/drone_battery_app_icon.ico  (relative to dist-electron/)
 * - Production: extraResources/icons/ (outside ASAR, via process.resourcesPath)
 */
function getIconPath(): string | undefined {
  try {
    if (IS_DEV) {
      const devIcon = path.join(__dirname, '..', 'public', 'drone_battery_app_icon.ico');
      return fs.existsSync(devIcon) ? devIcon : undefined;
    }
    // extraResources copies icon to resources/icons/
    const prodIcon = path.join(process.resourcesPath, 'icons', 'drone_battery_app_icon.ico');
...
ipcMain.handle('download-and-install-update', async (event, { downloadUrl, fileName }: { downloadUrl: string; fileName: string }) => {
  try {
    ...
    log('INFO', `Update downloaded successfully to ${targetPath}. Launching installer...`);
